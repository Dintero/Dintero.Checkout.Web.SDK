...
describe("dintero.embed", () => {
    ...
    it("Adds backdrop to DOM when a PopOutOpened message is received", async () => {
        const script = `
            // Tell the SDK to create the backdrop
            emit({
                type: "PopOutOpened",
            });
        `;
    ...
    it("Removes backdrop from DOM when a PopOutClosed message is received", async () => {
        const script = `
            // Tell the SDK to create the backdrop
            emit({
                type: "PopOutOpened",
            ...
            window.setTimeout(()=>{
                // Tell the SDK to remove the backdrop
                emit({
                    type: "PopOutClosed",
                });
            }, 50);
        ...
        const result: void = await new Promise((resolve, reject) => {
            ...
            // Wait for backdrop to be created and removed.
            sleep(100).then(() => {
                const backdrop = document.getElementById('dintero-checkout-sdk-backdrop');
                if (backdrop) {
                    reject();
    ...
    it("Emits ClosePopOut to iframe when close button is clicked", async () => {
        const script = `
            // Tell the SDK to create the backdrop
            emit({
                type: "PopOutOpened",
            ...
            window.addEventListener("message", function(event){
                // Abusing the SessionPaymentAuthorized event to send
                // received messages back to the SDK so we can assert it
                emit({
                    type: "SessionPaymentAuthorized",
                    transaction_id: event.data.type
                });
            }, "*");
        `;
        const getSessionUrlStub = sinon
        ...
        const result: {
            ...
            sleep(50).then(() => {
                ...
                const closeButton = document.getElementById('dintero-checkout-sdk-backdrop-close');
                closeButton.click();
            })
            sleep(100).then(reject);
        });
        ...
        expect(result.event.transaction_id).to.equal("ClosePopOut");
        getSessionUrlStub.restore();
        (await checkout).destroy();
    });


    it("Emits FocusPopOut to iframe when the backdrop is clicked", async () => {
        const script = `
            // Tell the SDK to create the backdrop
            emit({
                type: "PopOutOpened",
        ...
        expect(result.event.transaction_id).to.equal("FocusPopOut");
        getSessionUrlStub.restore();
        (await checkout).destroy();
    });
});
